...
# We initialize them with dummy values; they will be re-initialized in main()
...
def train(S: torch.Tensor, A: torch.Tensor, returns: torch.Tensor, 
          GAMMA: float, OPT: torch.optim.Optimizer, V_OPT: torch.optim.Optimizer):
    """
    Performs one step of policy gradient update using REINFORCE with Baseline.
    ...
    for _ in range(POLICY_TRAIN_ITERS):
        ...
        # 1. Calculate state values: V(s_t)
        state_values = V(S).squeeze()
        # 2. Calculate Value loss (Critic loss): MSE(G_t, V(s_t))
        # G_t is the 'returns' tensor
        value_loss = torch.nn.functional.mse_loss(state_values, returns)
        # 3. Update Critic
        value_loss.backward() # Gradients for V
        V_OPT.step()
...
def main():
    ...
    for epi in pbar:
        ...
        # Train both networks
        train(S_tensor, A_tensor, returns_tensor, GAMMA, OPT, V_OPT)
    ...
    plot_path = os.path.join(image_dir, f"reinforce-baseline-{args.mode}.png")
    
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(last25Rs)), last25Rs, 'b')
    plt.xlabel('Episode')
    plt.ylabel('Reward (averaged over last 25 episodes)')
    plt.title(f"REINFORCE with Baseline, mode: {args.mode}")
    plt.grid(True)
